			...
			<div class="libros">
				<div class="col-sm-6">
					<div class="Nombre">
						<?php
							//Nombre de genero
							$consultaNombre="SELECT NombreAutor, ApellidoAutor FROM autor WHERE idAutor=$dato";
							$resultadoNombre=mysqli_query($conn,$consultaNombre);
							$arrayNombre=mysqli_fetch_array($resultadoNombre);
							$NombreA=$arrayNombre['NombreAutor'];
							$ApellidoA=$arrayNombre['ApellidoAutor'];
						?>
						<h3 id="nombreG"><?php echo $NombreA." ".$ApellidoA; ?><h3> <br>
					</div>
				</div>
				<?php

						//Encuentro todos los Libros
						$consulta="SELECT idLibro FROM autores_por_Libro where idAutor=$dato";
						$resultado=mysqli_query($conn,$consulta);
